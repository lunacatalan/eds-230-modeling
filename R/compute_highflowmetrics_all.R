...
#' @param wts (vector of 4 for annual_max_err, annual_max_corr, max_month_cor,high_month_err)
...
compute_highflowmetrics_all = function(m,o, month, day, year,wy, high_flow_months=3,
  ...
  if (is.null(max_err_max_month))
  { max_err_max_month = 0.5*mean(high$obs)}
  
  annual_max_err_trans = max(0,(1-abs(annual_max_err/max_err_annual_max) ))
  high_month_err_trans = max(0,(1-abs(high_month_err/max_err_max_month) ))
